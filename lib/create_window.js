...
const {SHORTCUTS} = require('../config')
...
function setAcceleratorTemplate () {
  ...
  for (let i = 0; i < limit; i++) {
    template[i].accelerator = SHORTCUTS.CLIPBOARD_COPY_BY_POSITION.replace('{pos}', i)
  }
}
...
function getCopiedFromDbWithLabel () {
  ...
  copied = copied.map(currentText => {
    return {
      text: currentText,
      label: truncateCurrentText(currentText, SHORTCUTS.HISTORY_STRING_LENGTH, false),
    }
  })
  return { copied, length }
}
...
function registerShortcuts () {
  for (let i = 1; i < 10; i++) {
    globalShortcut.register(SHORTCUTS.CLIPBOARD_COPY_BY_POSITION.replace('{pos}', i), () => shortcutAction(i))
  }
  globalShortcut.register(SHORTCUTS.CLIPBOARD_SHOW_MANAGER, () => {
    tray.focus()
  })
}

function registerGlobalShortcutForClipboardWindow () {
  globalShortcut.register(SHORTCUTS.CLIPBOARD_SHOW_HISTORY, () => {
    ...
    clipWin.webContents.on('did-finish-load', () => {
      clipWin.webContents.send('clipboardContents', getCopiedFromDbWithLabel())
    })
  })
}
